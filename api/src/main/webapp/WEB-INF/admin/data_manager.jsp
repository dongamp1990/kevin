				...
				<div class="page-content">
					...
					<div class="page-bar">
						<ul class="page-breadcrumb">
							<li><i class="fa fa-home"></i> <a href="javascript:void(0)">资料管理</a></li>
						</ul>
									...
									<div id="sample_1_wrapper" class="dataTables_wrapper no-footer">
											...
											<div class="col-md-6 col-sm-6">
												<div id="sample_1_filter" class="dataTables_filter">
													<label>搜索：<input type="search"
														...
														placeholder="QQ号码 按回车搜索" aria-controls="sample_1" id="qtName" value="${qtName }"/></label>
												</div>
											</div>
										</div>
										<div class="table-scrollable">
											<table
												...
												<tbody>
													<c:if test="${not empty customerPage}">
														<c:forEach var="customer" items="${customerPage.list }">
															<tr>
																...
																<td class="numeric">
																	${customer.illegality_record }
																</td>
																<td class="numeric">
																	<f:formatDate value="${customer.create_date }" pattern="yyyy-MM-dd HH:mm:ss"/>
																</td>
																<td class="numeric">
																	<a onclick="deleteData(${customer.id});"><span aria-hidden="true" class="icon-trash" title="删除"></span></a>
																	<a onclick="updateData(${customer.id});"><span aria-hidden="true" class="icon-wrench" title="修改"></span></a>
																</td>
															</tr>
														</c:forEach>
													</c:if>
...
<!-- 											</div> -->
<!-- 										</div> -->
											<page:pager pageSize="${customerPage.pageSize }" requestUrl="${pageContext.request.contextPath}/admin/data_manager?currentPage=page&pageSize=${customerPage.pageSize }" 
	                            		totalRecord="${customerPage.totalRow }" currentPage="${customerPage.pageNumber}"/>		
									</div>
								</div>
							</div>
...
<![endif]-->
	...
	<script>
			...
			document.onkeydown=function(event){
	          var e = event || window.event || arguments.callee.caller.arguments[0];
	              if(e && e.keyCode==13){ // enter 键
	                 if($("#qtName").val() != null && $("#qtName").val() != "") {
	         			var url = "${pageContext.request.contextPath}/admin/data_manager?&currentPage=1&pageSize="+${pageSize}+"&qtName=" + $("#qtName").val();
		...
		function pageLimitChange() {
			var limit = $("#limit").val();
			var url = "${pageContext.request.contextPath}/admin/data_manager?&currentPage=1&pageSize="+limit+"&qtName=" + $("#qtName").val();
			window.location.href = url;
		}
		
		function dataInput() {
			var url = "${pageContext.request.contextPath}/admin/data_manager_input";
			window.location.href = url;
		}
		function updateData(id) {
			var url = "${pageContext.request.contextPath}/admin/update_data_manager_input?id="+id;
			window.location.href = url;
		}
		function deleteData(id) {
			if(confirm("操作不可恢复，您确定操作吗?")) {
				var url = "${pageContext.request.contextPath}/admin/delete_customer_info?id="+id;
				$.get(url, function(data) {
					if(data.status == "ok") {
						var url = "${pageContext.request.contextPath}/admin/data_manager?currentPage=${currentPage}&pageSize=${pageSize}&qtName=" + $("#qtName").val();
						window.location.href = url;
					}
				});
			}
